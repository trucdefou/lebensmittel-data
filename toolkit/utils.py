...
def clean_text(s):
    ...
    s = s.strip()
    s = ' '.join(s.split())  # collapse any whitespace sequences to a single space
    return s
...
def plot_expense_line(df, date_col='datum', price_col='preis', group_by=None, palette='tab20'):
    df = df.copy()

    if group_by:
        plot_df = df.groupby([date_col, group_by], observed=True)[price_col].sum().reset_index()
    else:
        plot_df = df.groupby(date_col)[price_col].sum().reset_index()
        plot_df['group'] = 'Total'
        group_by = 'group'

    plt.figure(figsize=(16, 8))  # wider figure for better legend space
    sns.lineplot(
        data=plot_df,
        x=date_col,
        y=price_col,
    ...
    plt.title("Expenses Over Time" + (f" by {group_by.capitalize()}" if group_by != 'group' else ""))
    plt.xlabel("Date")
    plt.ylabel("Expenses (€)")
...
def graph_pie(df, column_names, num_columns=3, figsize=(14, 10)):
    """
    Generate pie charts for selected DataFrame columns.
    
    Parameters:
    - df: pandas DataFrame
    - column_names: list of column names to plot
    - num_columns: number of columns in the subplot grid
    - figsize: figure size for the whole plot
    """
    total_charts = len(column_names)
    num_rows = total_charts // num_columns + int(total_charts % num_columns > 0)

    fig, axes = plt.subplots(nrows=num_rows, ncols=num_columns, figsize=figsize)
    axes = axes.flatten() if total_charts > 1 else [axes]
